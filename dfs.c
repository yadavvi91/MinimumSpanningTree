 ...
 * \return void
 */
void perform_dfs(adj_list *adjacency_list)
{
    ...
    fprintf(stdout, "The sets are: (In reverse order)\n");
    for (int i = 0; i < adjacency_list->no_vert; i++) {
 ...
 */
boolean find_a_cycle(adj_list *adjacency_list)
{
    stack *stack_inst = stack_init();
    ...
    while (no_of_vertices > 0) {
        ...
        while (node_of_vertex) {
            if (marked[node_of_vertex->to] == TRUE) {
                ...
                fprintf(stdout, "%5d", node_of_vertex->from);
                do {
                    fprintf(stdout, "%5d", edge_to[i]);
                    i = edge_to[i];
